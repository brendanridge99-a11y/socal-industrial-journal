...
def add_new_articles(data, new_articles):
    ...
    for article in new_articles:
        ...
        try:
            # Parse the date (format: "May 12, 2026")
            article_date = datetime.strptime(article_date_str, "%B %d, %Y")
            
            # Calculate age in hours (more precise than days)
            hours_old = (datetime.utcnow() - article_date).total_seconds() / 3600
            
            # Reject if older than retention window
            ...
            # If date parsing fails, REJECT the article (don't risk adding old content)
            skipped_count += 1
