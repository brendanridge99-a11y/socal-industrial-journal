...
# Used to tag local-deals articles with their submarket
# ─────────────────────────────────────────────────────────────
...
Categorize each article into ONE primary category, then optionally list up to 2 secondary categories where the article also has strong relevance:
...
SECONDARY CATEGORY RULES — only add a secondary category if the article has STRONG, DIRECT relevance to it (not just tangential). Maximum 2. Examples:
- A Prologis lease in Ontario → primary: local-deals, secondary: ["industries"]
- A Fed rate decision → primary: economics, secondary: [] (too broad to tag secondarily)
- Amazon building a Fontana fulfillment center → primary: local-deals, secondary: ["business", "supply-chain"]
- A CARB truck emissions ruling → primary: politics, secondary: ["supply-chain"]
...
def add_new_articles(data, new_articles):
    ...
    for article in new_articles:
        ...
        valid_cats = {'economics','industries','business','tech','politics','supply-chain','local-deals'}
        raw_secondary = article.get('secondary_categories', [])
        if isinstance(raw_secondary, list):
            article['secondary_categories'] = [
                c for c in raw_secondary
                if c in valid_cats and c != article.get('category')
            ][:2]  # max 2
        else:
            article['secondary_categories'] = []

        # Tag local-deals articles with their submarket(s)
        if article.get('category') == 'local-deals':
            article['submarkets'] = tag_submarket(article)

        # ── Option 3: Local-deals overflow ───────────────────
        # If a non-local article mentions a SoCal city, create a
        # lightweight overflow copy in local-deals so it surfaces
        # in the Local tab without cluttering other categories.
        if article.get('category') != 'local-deals':
            submarkets = tag_submarket_any(article)
            if submarkets:
                overflow = dict(article)
                overflow['id']          = article_id + '_loc'
                overflow['category']    = 'local-deals'
                overflow['submarkets']  = submarkets
                overflow['is_overflow'] = True   # flag — shown with subtle indicator
                overflow['overflow_from'] = article.get('category')
                # Only add overflow if not already a duplicate in local-deals
                if overflow['id'] not in existing_ids:
                    data['articles'].append(overflow)
                    existing_ids.add(overflow['id'])
                    print(f"   ↳ Local overflow ({", ".join(submarkets)}): {article['headline'][:50]}...")
...
def tag_submarket(article):
    """
    Assign submarket tag(s) to a local-deals article.
...
def tag_submarket_any(article):
    """
    Check ANY article (any category) for SoCal city mentions.
    Returns list of matched submarket keys, or [] if none found.
    Used to create local-deals overflow copies of nationally-categorised
    articles that have strong local relevance.
    """
    return _match_submarkets(article)

def _match_submarkets(article):
    """
    Core city-matching logic shared by both tagging functions.
...
<footer class="site-footer">
  ...
  <p class="cat-summary-text">{long_summary}</p>
            ...
            def get_item_submarkets(item):
                """Get submarket tags for a display item (cluster or solo article)."""
                if item.get('is_cluster'):
                    # Union of all submarkets across articles in cluster
                    sm = set()
            ...
            def sm_attr(item):
                """Build data-submarket attribute string for a card."""
                if cat_key != 'local-deals':
                    return ''
  ...
  {sources_html}
                ...
                overflow_badge = f'<span class="overflow-badge">via {section_titles.get(item.get("overflow_from",""), "")}</span>' if is_overflow else ''
                ...
                # Cross-category tags (shown on non-overflow cards only)
                cross_cats = item.get('secondary_categories', []) if not is_overflow else []
                if cross_cats:
                    tag_links = ''.join(
                        f'<a href="{c}.html" class="cross-cat-tag">{nav_short_titles.get(c, c)}</a>'
                        for c in cross_cats
                    )
...
{rest_section}'''
        ...
        # ── Submarket filter bar (local-deals only) ───────────
        submarket_bar_html = ''
        submarket_js = ''
        if cat_key == 'local-deals' and count > 0:
            # Count articles per submarket for button labels
...
function filterSM(btn) {
  ...
  cards.forEach(card => {
    if (sm === 'all') {
      ...
      // Show if card matches selected submarket OR is tagged 'all' (unmatched)
  ...
  // Hide empty section labels if all cards in that section are hidden
