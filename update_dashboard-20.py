...
def generate_summaries(data):
    ...
    for cat_key, cat_title in section_titles.items():
        ...
        prompt = f"""You are writing a brief editorial summary for a section of a news digest read by Southern California industrial real estate brokers.

Below are the {cat_title} news articles from the past 72 hours:

{article_context}

Write a 2-3 sentence summary of what is most important in this category right now. Be direct and factual — report the key developments, specific figures, and notable trends. Do not speculate, advise, or tell readers what to do with this information. Write as if you are a sharp editor summarizing the section for a busy professional who needs to quickly understand the state of play."""
    ...
    if all_summaries_context:
        bottom_line_prompt = f"""You are writing the lead editorial note for a news digest read by Southern California industrial real estate brokers.

Below are brief summaries of today's top news across all categories:

{all_summaries_context}

Write 2-3 sentences that capture the single most important big-picture story across all these categories — the overarching theme or most consequential development that a busy broker needs to understand today. Be direct, specific, and grounded in the facts above. Do not speculate or advise. This should read like a sharp, confident summary from a senior editor — not a list, not bullet points, just clean prose."""
