...
def search_new_articles(existing_articles):
    """
    Multi-call search architecture — 13 focused calls per run.

    CADENCE:
    - Calls 1–10 (high/medium velocity): every run (every 2 hours)
    - Calls 11–13 (regional papers, lower velocity): every other run
      (determined by whether the current UTC hour is even)

    Each call targets a specific source group with focused instructions.
    Results are merged and deduplicated before returning.
    """
    print("🔍 Searching for new articles (multi-call architecture)...")

    from datetime import timezone
    utc_hour = datetime.now(timezone.utc).hour
    run_regional = (utc_hour % 2 == 0)  # regional calls on even hours only

    client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)

    # ── Define all 13 call groups ─────────────────────────────
...
- Bloomberg (bloomberg.com) — all sections: markets, economics, technology, real estate, law, logistics
- Bloomberg Law (bloomberg.com/law) — regulatory actions, labor rulings, NLRB decisions, court decisions affecting business
- Bloomberg Intelligence — sector research notes and analyst commentary relevant to industrial real estate, logistics, or tenant industries

Bloomberg publishes dozens of relevant stories per day. Search broadly across all its sections.
Pay special attention to any coverage of: Fed and monetary policy, CMBS and CRE lending,
supply chain and freight markets, industrial REIT performance, tariffs and trade policy,
and major corporate operational decisions by tenant industries.""",
            5000
        ),
...
- Wall Street Journal (wsj.com) — all sections: markets, economy, business, real estate, logistics
- WSJ Logistics Report (wsj.com/logistics) — dedicated freight and supply chain coverage published daily
- WSJ Pro Bankruptcy (wsj.com/pro/bankruptcy) — tenant distress signals: Chapter 11 filings, restructurings, store closures with real estate implications

IMPORTANT: Check wsj.com/logistics specifically — this is a dedicated beat and produces
daily coverage of trucking, ports, freight rates, and supply chain that frequently gets
missed in broad searches. Also check wsj.com/pro/bankruptcy for any retail or industrial
tenant bankruptcy filings that signal space vacancies or lease rejections.""",
            4000
        ),
        ...
        (
            "Reuters",
            """SOURCES FOR THIS SEARCH — Reuters only:
- Reuters (reuters.com) — global wire service: economy, markets, trade, corporate news
- Reuters California (reuters.com/world/us/california) — California-specific economic, regulatory, and business news

IMPORTANT: Check reuters.com/world/us/california specifically for California-focused coverage
that may not surface in the main Reuters feed. This section frequently covers California
regulatory actions (CARB, labor law), port activity, and major California employer moves
that are directly relevant to SoCal industrial real estate.""",
            3500
        ),
        ...
        (
            "Politico",
            """SOURCES FOR THIS SEARCH — Politico only:
- Politico (politico.com) — federal policy, trade legislation, regulatory pipeline, government affairs

Politico covers the policy actions that directly shape industrial real estate operating
and development environments. Look specifically for:
- Trade and tariff policy developments
- Federal infrastructure bill funding and implementation
- NLRB and labor regulatory actions
- Transportation and logistics regulations (DOT, FAA, FMC)
- Environmental regulatory actions (EPA, stormwater, emissions)
- Tax policy changes affecting CRE investment (1031, opportunity zones)
- Defense spending and contract awards affecting aerospace/manufacturing tenants""",
            3000
        ),

        # ── CALL 5: FreightWaves + JOC + Supply Chain Dive ───
        (
            "Freight & Logistics Trade Press",
            """SOURCES FOR THIS SEARCH:
- FreightWaves (freightwaves.com) — real-time freight market data, trucking rates, capacity, spot market
- Journal of Commerce / JOC (joc.com) — port operations, ocean freight, intermodal, global trade flows
- Supply Chain Dive (supplychaindive.com) — supply chain strategy, disruption, inventory dynamics, 3PL

These three publications are the core logistics intelligence sources. Search all three
thoroughly. They publish multiple relevant articles per day covering:
- Container rates and ocean freight market conditions
- LA/Long Beach port volumes, congestion, and labor
- Truckload and LTL rate environment
- Inventory-to-sales ratios and warehouse demand signals
- 3PL sector moves and capacity
- Supply chain disruptions (weather, geopolitical, labor)
- Nearshoring and reshoring effects on cargo patterns""",
            4000
        ),

        # ── CALL 6: CRE Deal Flow ─────────────────────────────
        (
            "CRE Deal Flow",
            """SOURCES FOR THIS SEARCH:
- CoStar (costar.com) — industrial market data, transaction records, submarket analytics
- Connect CRE (connect.media) — deal announcements, project news, transaction coverage
- BisNow (bisnow.com) — CRE news and transaction coverage across major metros
- The Real Deal (therealdeal.com) — breaking CRE deals and development news
- Commercial Observer (commercialobserver.com) — CRE finance, CMBS, lending, investment sales

These sources are the primary deal-flow pipeline for the SoCal page. Search specifically for:
- Industrial lease transactions in IE, LA, OC, and San Diego submarkets
- Industrial investment sales and portfolio transactions
- Industrial development announcements: ground breakings, spec projects, BTS deals
- Market reports with SoCal industrial vacancy, rent, and absorption data
- CMBS and CRE lending activity related to industrial assets
- Institutional capital targeting industrial real estate

These publications update throughout the day — search aggressively.""",
            4500
        ),

        # ── CALL 7: LA Times + LABJ ───────────────────────────
        (
            "LA Times + LA Business Journal",
            """SOURCES FOR THIS SEARCH:
- Los Angeles Times (latimes.com) — California's largest paper: business, economy, labor, real estate, port coverage
- Los Angeles Business Journal (labusinessjournal.com) — LA-focused business news, deals, and employer moves

The LA Times covers the Port of LA/Long Beach extensively and publishes frequent coverage
of Southern California economic conditions, major employer moves, labor actions, and
state regulatory developments. Look for:
- Port of LA/Long Beach cargo volumes, labor, and congestion
- Major employer expansions or contractions in SoCal
- California economic indicators and business conditions
- Labor actions affecting logistics or manufacturing operations
- State and local regulatory developments affecting industrial operations
- Development and zoning news in LA County

The LABJ focuses on LA-specific business deals, CRE transactions, and company news.""",
            3500
        ),

        # ── CALL 8: CRE Secondary Tier ────────────────────────
        (
            "CRE Secondary Tier",
            """SOURCES FOR THIS SEARCH:
- GlobeSt (globest.com) — national CRE news with strong industrial and investment coverage
- Propmodo (propmodo.com) — proptech, industrial innovation, and real estate technology
- Green Street (greenstreet.com) — institutional CRE research, REIT analysis, cap rate intelligence

GlobeSt publishes multiple daily stories on industrial real estate transactions and market
trends nationally and in SoCal. Look for lease deals, investment sales, and market analysis.

Propmodo covers warehouse automation, industrial proptech, and technology changing how
industrial space is designed and operated — assign to appropriate category per tiebreaker rules.

Green Street publishes research notes on REIT performance, cap rates, and industrial market
conditions — these are high-value Economy or SoCal articles when available.""",
            3000
        ),

        # ── CALL 9: Policy + Specialty Logistics + IE Local ───
        (
            "Policy + Specialty Logistics + IE Local",
            """SOURCES FOR THIS SEARCH:
- CalMatters (calmatters.org) — California policy and legislation: CARB, AB5, warehouse worker rules, labor law
- DC Velocity (dcvelocity.com) — distribution center operations, warehouse management, 3PL industry
- Pacific Maritime Magazine (pacmar.com) — LA/Long Beach port operations, labor, container shipping
- IE Business Daily (iebusinessdaily.com) — Inland Empire business news, economic development, local deals

CalMatters is the authoritative source on California regulatory actions affecting industrial
operations and development. Search specifically for CARB rulemakings, warehouse ordinances,
AB5 enforcement, minimum wage changes, and any legislation affecting industrial tenants.

Pacific Maritime covers the San Pedro Bay port complex in depth — labor negotiations,
automation investments, volume data, and drayage capacity.

IE Business Daily covers the Inland Empire market specifically — search for local deals,
employer moves, development announcements, and city council decisions affecting industrial zoning.""",
            3500
        ),

        # ── CALL 10: Manufacturing Verticals + SD Business ────
        (
            "Manufacturing + San Diego Business",
            """SOURCES FOR THIS SEARCH:
- Industry Week (industryweek.com) — manufacturing operations, reshoring trends, factory investment, automation
- Modern Materials Handling (mmh.com) — warehouse operations, automation, distribution technology
- Food Logistics (foodlogistics.com) — cold chain, food distribution, grocery supply chain
- San Diego Business Journal (sdbj.com) — San Diego business news, CRE, economic development

Industry Week and MMH cover the manufacturing and warehouse operations that occupy industrial
space — search for reshoring announcements, factory investments, automation deployments,
and industry-wide operational shifts.

Food Logistics covers the cold storage and food distribution sector — a major industrial
demand driver in the IE and LA markets specifically.

San Diego Business Journal covers SD CRE deals, employer moves, cross-border manufacturing
activity with Mexico, and port of San Diego developments.""",
            3000
        ),

        # ── CALL 11: Western IE + Eastern IE + SB Corridor ───
        # Runs on even UTC hours only
        (
            "IE Regional Papers — West & East Corridors",
            """SOURCES FOR THIS SEARCH — SoCal regional newspapers covering the Inland Empire:
- Daily Bulletin / Inland Valley Daily Bulletin (dailybulletin.com) — Ontario, Rancho Cucamonga, Chino, western IE
- The Press-Enterprise (pe.com) — Riverside County: Riverside, Corona, Moreno Valley, Perris
- The Sun / San Bernardino Sun (sbsun.com) — San Bernardino, Fontana, Rialto, eastern IE
- Redlands Daily Facts (redlandsdailyfacts.com) — Redlands, Loma Linda, eastern San Bernardino Valley
- Los Angeles Daily News (dailynews.com) — San Fernando Valley and greater LA County

These are local newspapers covering the communities where most SoCal industrial development
occurs. They are the primary source for:
- Local city council decisions affecting industrial zoning or development
- Groundbreaking ceremonies and facility opening announcements
- Employer moves into or out of specific cities
- Local opposition to or support for warehouse and logistics development
- Development permit approvals for industrial projects
- Local employer layoffs or closures with real estate implications

Search for any story involving: warehouses, distribution centers, manufacturing facilities,
logistics operations, industrial parks, freight operations, truck traffic, or major employers.""",
            3000
        ),

        # ── CALL 12: South Bay + SGV Corridor ────────────────
        # Runs on even UTC hours only
        (
            "LA Regional Papers — South Bay & SGV",
            """SOURCES FOR THIS SEARCH — SoCal regional newspapers covering the LA Basin:
- Press-Telegram (presstelegram.com) — Long Beach, Carson, South Bay industrial corridor
- Daily Breeze (dailybreeze.com) — Torrance, Hawthorne, El Segundo, South Bay
- San Gabriel Valley Tribune (sgvtribune.com) — City of Industry, El Monte, SGV industrial corridor
- Pasadena Star-News (pasadenastarnews.com) — Pasadena, eastern San Gabriel Valley
- Whittier Daily News (whittierdailynews.com) — Whittier, Santa Fe Springs, southeast LA industrial corridor

These newspapers cover the LA Basin industrial corridors adjacent to the ports.
Long Beach, Carson, and the South Bay are critical for port-adjacent industrial.
City of Industry, El Monte, and Santa Fe Springs are major LA industrial submarkets.

Search for: warehouse and distribution center news, port-related industrial activity,
employer moves, zoning and development decisions, logistics facility openings or closures,
and any industrial real estate transactions or development news.""",
            3000
        ),

        # ── CALL 13: OC + San Diego Regional ─────────────────
        # Runs on even UTC hours only
        (
            "OC & San Diego Regional Papers",
            """SOURCES FOR THIS SEARCH — Regional newspapers covering Orange County and San Diego:
- Orange County Register (ocregister.com) — all of Orange County: Anaheim, Irvine, Fullerton, Santa Ana industrial
- San Diego Union-Tribune (sandiegouniontribune.com) — San Diego County: Otay Mesa, Chula Vista, Carlsbad, industrial parks

The OC Register covers Orange County industrial markets including the Anaheim-Fullerton
corridor, John Wayne Airport adjacency, and Irvine industrial parks.

The SD Union-Tribune covers the San Diego industrial market including the critical Otay Mesa
cross-border logistics hub, Carlsbad tech manufacturing, and Chula Vista industrial parks.

Search for: industrial leases, development news, employer moves, cross-border logistics
activity (especially Otay Mesa), zoning and permitting news, and market conditions.""",
            2500
        ),
    ]
    ...
    for i, (label, sources_block, max_tok) in enumerate(CALL_GROUPS):
        call_num = i + 1
        is_regional = call_num >= 11  # calls 11, 12, 13

        if is_regional and not run_regional:
            print(f"   ⏭️  [{label}] Skipping (regional — odd-hour run)")
            continue

        print(f"\n   📡 Call {call_num}/{'13' if run_regional else '10'}: {label}")
        articles = _run_search_call(client, sources_block, label, max_tokens=max_tok)
        all_raw_articles.extend(articles)
        calls_run += 1

    print(f"\n   📊 {calls_run} calls completed — {len(all_raw_articles)} raw articles before dedup")

    # ── Batch-level deduplication (within this run's results) ─
    # add_new_articles handles dedup against the database;
    # this pass removes exact duplicates within the batch itself
    # (same story found by multiple calls — expected and fine)
    seen_headlines = set()
    seen_urls = set()
    deduped = []
    ...
    print(f"   ✅ {len(deduped)} articles passed to database")
...
def main():
    ...
    from datetime import timezone
    utc_hour_main = datetime.now(timezone.utc).hour
    regional_ran = (utc_hour_main % 2 == 0)
    calls_this_run = 13 if regional_ran else 10
    ...
    print(f"   Search calls this run: {calls_this_run} ({'+ regional' if regional_ran else 'core only — regional runs next hour'})")
