...
Write exactly 1 sentence highlighting the most notable deal, transaction, or development across these stories. Be specific — include the property, company, location, or dollar figure that makes it newsworthy. Do not speculate or advise."""
        else:
            prompt = f"""Below are {cat_title} news articles from the past 72 hours:

{article_context}

Write exactly 1 sentence summarizing the broad state of play in {cat_title} right now — the overarching theme or dominant trend across these stories. Stay at the macro level. Do not reference specific deals, companies, or properties unless they represent the defining story of the period. Do not speculate or advise."""
...
'''
    ...
    # ── Group & sort articles ──────────────────────────────────
    categories = {k: [] for k in ['economics','industries','business','tech','politics','supply-chain','local-deals']}
    for article in data['articles']:
        cat = article.get('category', 'industries')
        if cat in categories:
            categories[cat].append(article)
    ...
    def generate_header(active_page='home'):
        ...
        nav_items = []
        for cat_key, cat_title in section_titles.items():
            short = cat_title.split(' & ')[0].split()[0]
...
<footer class="site-footer">
  ...
  <div>Rolling 72-hour window &nbsp;·&nbsp; Updates every 2 hours &nbsp;·&nbsp; Summaries refreshed daily</div>
    ...
    def generate_category_page(cat_key, cat_title):
        ...
        if count == 0:
            articles_html = '<div class="empty-state">No stories in this category yet — check back soon.</div>'
        else:
            cards = []
            for article in arts:
                is_paywalled = article.get('source_type','free') == 'paywalled'
                paywall = '<span class="paywall-badge">🔒 Subscriber</span>' if is_paywalled else ''
                access  = f'&nbsp;&nbsp;<a href="mailto:{YOUR_EMAIL}" style="font-family:\'Jost\',sans-serif;font-size:0.62rem;color:var(--muted);text-decoration:none;letter-spacing:0.05em;">Request access →</a>' if is_paywalled else ''

                cards.append(f'''<div class="article-card" style="--cat-color:{color};">
